...
class LPPLS(object):
    ...
    def plot_confidence_indicators_one(self, res):
        """
        Args:
        ...
        """
        #引用箇所--------------------------------
        sxmin=736603#'2017-10-01'
        sxmax=738187#'2022-02-01'
        xmin = sxmin
        xmax = sxmax
        #----------------------------------------
        ...
        ax1 = plt.plot(nrows=1, ncols=1, sharex=True, figsize=(20, 5))
    ...
    def plot_confidence_indicators_2(self, res):
        """
        Args:
            res (list): result from mp_compute_indicator
            condition_name (str): the name you assigned to the filter condition in your config
        ...
        ax1_0.plot(ts, res_df['pos_conf'], label='bubble indicator (pos)', color='red', alpha=0.5)
        ax1_1.plot(ts, res_df['neg_conf'], label='bubble indicator (neg)', color='green', alpha=0.5)
        ...
        # set labels
        ax1.set_ylabel('Weekly Average Price: ln(USD)')
        ax2.set_ylabel('Weekly Average Price: ln(USD)')
