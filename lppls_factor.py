...
class LPPLS_FACTOR(object):
    ...
    @staticmethod
    @njit
    def lppls_factor(t, v, tc, m, w, a, b, c1, c2, d):
        return a + np.power(tc - t, m) * (b + ((c1 * np.cos(w * np.log(tc - t))) + (c2 * np.sin(w * np.log(tc - t))))) + d * v
    ...
    def matrix_equation(observations, tc, m, w):
        ...
        ji_pow_2 = np.power(ji, 2)

        figi = np.multiply(fi, gi)
        fihi = np.multiply(fi, hi)
        gihi = np.multiply(gi, hi)
        fiji = np.multiply(fi, ji)
        giji = np.multiply(gi, ji)
        hiji = np.multiply(hi, ji)

        yi = P
        yifi = np.multiply(yi, fi)
        yigi = np.multiply(yi, gi)
        yihi = np.multiply(yi, hi)
        yiji = np.multiply(yi, ji)

        matrix_1 = np.array([
            [N,          np.sum(fi),       np.sum(gi),       np.sum(hi),         np.sum(ji)],
            [np.sum(fi), np.sum(fi_pow_2), np.sum(figi),     np.sum(fihi),       np.sum(fiji)],
            [np.sum(gi), np.sum(figi),     np.sum(gi_pow_2), np.sum(gihi),       np.sum(giji)],
            [np.sum(hi), np.sum(fihi),     np.sum(gihi),     np.sum(hi_pow_2),   np.sum(hiji)],
            [np.sum(ji), np.sum(fiji),     np.sum(giji),     np.sum(hiji),       np.sum(ji_pow_2)],
        ])
    ...
    def fit(self, max_searches, minimizer='Nelder-Mead', obs=None):
        ...
        while search_count < max_searches:
            ...
            try:
                tc, m, w, a, b, c, c1, c2, d = self.estimate_params(obs, seed, minimizer)
                O = self.get_oscillations(w, tc, t1, t2)
                D = self.get_damping(m, w, b, c)
                return tc, m, w, a, b, c, c1, c2, d, O, D
    ...
    def estimate_params(self, observations, seed, minimizer):
        ...
        if cofs.success:
            ...
            for coef in ['tc', 'm', 'w', 'a', 'b', 'c', 'c1', 'c2', 'd']:
                self.coef_[coef] = eval(coef)
            return tc, m, w, a, b, c, c1, c2, d
        else:
    ...
    def plot_fit(self, show_tc=False):
        ...
        """
        tc, m, w, a, b, c, c1, c2, d = self.coef_.values()
        time_ord = [pd.Timestamp.fromordinal(d) for d in self.observations[0, :].astype('int32')]
        t_obs = self.observations[0, :] # 全ての行の0列目を取得
        v_obs = self.observations[2, :] # 全ての行の2列目を取得
        # ts = pd.to_datetime(t_obs*10**9)
        # compatible_date = np.array(ts, dtype=np.datetime64)
        lppls_fit = [self.lppls_factor(t, v, tc, m, w, a, b, c1, c2, d) for t, v in zip(t_obs, v_obs)]
        ...
        fig, (ax1) = plt.subplots(
            nrows=1, ncols=1, sharex=True, figsize=(14, 8))
        # fig.suptitle(
    ...
    def compute_indicators_factor(self, res, filter_conditions_config=None):
        pos_lst = []
        neg_lst = []
        pos_conf_lst = []
        neg_conf_lst = []
        ...
        for r in res:
            ...
            for fits in r['res']:
                ...
                tc_in_range = max(t2 - 60, t2 - 0.5 * (t2 - t1)
                                  ) < tc < min(t2 + 252, t2 + 0.5 * (t2 - t1))
    ...
    def plot_confidence_indicators_factor(self, res):
        """
        Args:
        ...
        fig, (ax1, ax2) = plt.subplots(
            nrows=2, ncols=1, sharex=True, figsize=(18, 10))
        # fig.suptitle(
        ...
        ax1_0.plot(
            ts, res_df['pos_conf'], label='bubble indicator (pos)', color='red', alpha=0.5)
        ...
        ax2_0.plot(
            ts, res_df['neg_conf'], label='bubble indicator (neg)', color='green', alpha=0.5)
    ...
    def mp_compute_nested_fits_factor(self, workers, window_size=80, smallest_window_size=20, outer_increment=5, inner_increment=2, max_searches=25, filter_conditions_config={}):
    ...
    def compute_nested_fits_factor(self, window_size=80, smallest_window_size=20, outer_increment=5, inner_increment=2,
                            max_searches=25):
        obs_copy = self.observations
        ...
        for i in range(0, obs_copy_len + 1, outer_increment):
            ...
            for j in range(0, window_delta, inner_increment):
                ...
                tc, m, w, a, b, c, c1, c2, d, O, D = self.fit(
        ...
        return xr.DataArray(
            ...
            coords=dict(
                ...
                windowsizes=range(smallest_window_size,
                                  window_size, inner_increment),
                params=['t2', 't1', 'a', 'b', 'c', 'd', 'm', '0', 'tc'],
            )
        )

    def _func_compute_nested_fits_factor(self, args):
